...
public class PhoneNumberClient {
    ...
    public static void main(String[] args) {
        PhoneNumberClient phoneNumberClient = new PhoneNumberClient();
        ManagedChannel channel = ManagedChannelBuilder.forAddress(host, port).usePlaintext().build();
        //客户端RPC流式通信必须建立异步stub事项
        phoneNumberClient.asyncStub = SmsServiceGrpc.newStub(channel);
        try{
            phoneNumberClient.createPhone();
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private void createPhone() throws InterruptedException {
        ...
        StreamObserver<SmsProto.PhoneNumberRequest> requestStreamObserver = asyncStub.createPhone(responseObserver);
        for (int i = 0; i < 10; i++) {
            //创建请求对象
